...
describe('Listener registration triggers the matching register*Listener RPC once', () => {
	test('registerConversionListener RPC fires exactly once, shared across onConversionDataSuccess and onConversionDataFail', () => {
		const { appsFlyer, nativeAppsFlyer } = freshModule();

		const removeA = appsFlyer.onConversionDataSuccess(jest.fn());
		...
		const removeB = appsFlyer.onConversionDataFail(jest.fn());
		expect(rpcMethodCalls(nativeAppsFlyer, 'registerConversionListener')).toHaveLength(1);
	...
	test('first onDeepLinking attach calls executeRpc with the canonical registerDeeplinkListener method name, only once', () => {
		const { appsFlyer, nativeAppsFlyer } = freshModule();

		const removeA = appsFlyer.onDeepLinking(jest.fn());
		expect(rpcMethodCalls(nativeAppsFlyer, 'registerDeeplinkListener')).toHaveLength(1);

		const removeB = appsFlyer.onDeepLinking(jest.fn());
		expect(rpcMethodCalls(nativeAppsFlyer, 'registerDeeplinkListener')).toHaveLength(1);
