...
appsFlyer.registerConversionListener = (
  ...
  rpcListenerBuckets.onInstallConversionData.push(onConversionDataSuccess);
  if (onConversionDataFail) {
    rpcListenerBuckets.onInstallConversionFailure.push(onConversionDataFail);
  }
  return function remove() {
    rpcListenerBuckets.onInstallConversionData = rpcListenerBuckets.onInstallConversionData.filter(
      (registered) => registered !== onConversionDataSuccess
    );
    if (onConversionDataFail) {
      rpcListenerBuckets.onInstallConversionFailure = rpcListenerBuckets.onInstallConversionFailure.filter(
...
appsFlyer.unregisterConversionListener = () => {
  ...
  rpcListenerBuckets.onInstallConversionData = [];
  rpcListenerBuckets.onInstallConversionFailure = [];
 ...
 */
appsFlyer.registerDeepLinkListener = createBucketListener("onDeepLink", ensureDeepLinkListenerRegistered);
...
export interface AppsFlyerApi {
   ...
   */
  registerDeepLinkListener(callback: (data: UnifiedDeepLinkData) => any): () => void;
  /**
   * Stop the native deep-link listener and clear all registered callbacks.
   * @platform android
   */
