      ...
      <ScrollView>
         {
            products.map((product, index) => {
               return (<Product key={index} product={product} goToProductScreen={goToProductScreen} addToCart={addProductToCart}/>)
            })
         }
      </ScrollView>
      <View style={styles.fab}>
         <FAB title="Your Cart" color={'#69c0ff'} onPress={() => goToCart(itemsInCart, removeProductFromCart, checkout)}/>
         <Badge value={itemsInCart.length} status="error" containerStyle={{
               position: 'absolute',
               top: -2,
               right: -2
            }}/>
