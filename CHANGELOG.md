...
- React Native >> Add iOS-only deep-link forwarding calls `handleOpenUrl(url, options?)` and `continueUserActivity(url, activityType?)` — no 6.x JS equivalent. Native reads a flat `url` plus `options`/`activityType`; `activityType` defaults natively to `NSUserActivityTypeBrowsingWeb`
...
- **`generateInviteLink`'s `deeplinkPath` deprecated and ignored** — no native counterpart on either platform; passing it now logs a warning. `customerID` and `baseDeeplink` still work: the plugin translates them to the native key names internally (iOS `referrerCustomerId`, Android `customerId`, both `baseDeepLink`).
