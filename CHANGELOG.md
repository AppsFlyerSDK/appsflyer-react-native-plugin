...
- React Native >> iOS now consumes native SDK 7.0.2 via `AppsFlyerRPC` 7.0.13; Android consumes `af-android-sdk` 7.0.1 via `af-android-plugin-bridge` 7.0.12
...
- React Native >> Move RPC method dispatch, wire-name/param resolution (including the `mediationNetwork`/`purchaseType` platform mappings), and event demuxing into the new `@appsflyer-sdk/js-core-plugin` dependency — `index.ts` now only supplies the `RNTransport` glue and re-exports that package's public API
...
- React Native >> Consolidate `index.js`/`index.d.ts` into a single typed `index.ts` entry point (`package.json`'s `main`/`types` now both point at it) — every plugin API is a native TypeScript function backed by `Promise`, not a hand-maintained `.d.ts` layered over untyped JS
- React Native >> Add `setUserPhone(countryCode, phoneNumber)` and `setUserFbLoginId(fbLoginId)` — hashed-PII setters with no 6.x equivalent
...
- React Native >> Strict mode (`$RNAppsFlyerStrictMode=true`) now pulls `AppsFlyerRPC/Strict` (no-IDFA build) instead of the plain `AppsFlyerRPC` pod; drops the podspec's unused `AppsFlyerFramework/Strict` dependency
